      ...
      <header className="relative z-20 flex items-center justify-between gap-4 px-4 sm:px-6 lg:px-10 py-4">
        ...
        <div className="flex items-center gap-2">
          <span className="ishema-pill hidden sm:inline-flex items-center gap-1.5 text-[11px] font-semibold rounded-full px-3 py-1.5">
            <span className="w-1.5 h-1.5 rounded-full bg-blue-400 badge-pulse" />
            v1.0 · Live
          </span>
          <button
            onClick={toggle}
        ...
        <div className="relative z-10 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          <div className="relative">
            ...
            <div className="lg:pl-8">
              ...
              {/* Three steps with the light-blue accent line */}
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <StepIcon className="w-4 h-4 text-blue-300" />
              ...
              <div className="ishema-rise ishema-rise-4 flex flex-wrap items-center gap-4 mt-7 text-[11px] text-slate-500">
                <span className="flex items-center gap-1.5">
                  <Shield className="w-3.5 h-3.5 text-blue-300" /> Built for RSSB
                </span>
                <span className="w-1 h-1 rounded-full bg-slate-600" />
                <span className="flex items-center gap-1.5">
                  <FileSpreadsheet className="w-3.5 h-3.5 text-blue-300" /> 141+ vouchers processed
                </span>
                <span className="w-1 h-1 rounded-full bg-slate-600" />
                <span className="flex items-center gap-1.5">
                  <ShieldAlert className="w-3.5 h-3.5 text-blue-300" /> Fraud detection built-in
                </span>
              </div>
            </div>
          </div>
            ...
            {FEATURES.map(({ icon: Icon, title, desc, badge, stat }) => (
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="w-10 h-10 rounded-lg flex items-center justify-center bg-blue-400/10 text-blue-300 feature-icon">
                    <Icon className="w-5 h-5" />
                  </div>
                  <div className="flex items-center gap-1.5">
                    {stat && (
                      <span className="text-[10px] font-semibold text-blue-200/90 bg-blue-400/10 border border-blue-400/20 px-2 py-0.5 rounded-full">
                        {stat}
                      </span>
                    )}
                    {badge && (
                      <span className="text-[10px] font-bold uppercase tracking-wider bg-blue-400/20 text-blue-200 px-2 py-0.5 rounded-full">
                        {badge}
                      </span>
                    )}
                  </div>
                ...
                <div className="feature-card-arrow mt-3 text-blue-300/60">
      ...
      <footer className="relative px-4 sm:px-6 lg:px-10 py-8 border-t border-white/5">
        <div className="max-w-7xl mx-auto flex flex-col items-center gap-3 text-center">
          <div className="footer-icon-divider">
            <Pill className="w-3.5 h-3.5 text-blue-300/60" />
            <Shield className="w-3.5 h-3.5 text-blue-300/60" />
            <ShieldCheck className="w-3.5 h-3.5 text-blue-300/60" />
          </div>
      ...
      <div className="p-4 bg-slate-50">
        ...
        <div className="grid grid-cols-3 gap-2.5 mb-3">
          {[
            ...
            { label: 'Flagged', value: '7', tone: 'text-blue-500' },
          ].map(k => (
          ...
          {[
            ...
            <div key={r.v} className="grid grid-cols-12 gap-2 px-3 py-1.5 text-[10px] border-t border-slate-100 items-center">
              ...
              <div className="col-span-2">
                <span className={`px-1.5 py-0.5 rounded text-[8px] font-bold ${r.ok ? 'bg-emerald-100 text-emerald-700' : 'bg-blue-100 text-blue-700'}`}>
                  {r.s}
                </span>
              </div>
              <div className={`col-span-2 text-right tabular-nums font-semibold ${r.ok ? 'text-slate-400' : 'text-blue-500'}`}>{r.d}</div>
            </div>
          ))}
        </div>
